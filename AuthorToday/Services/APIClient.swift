...
actor APIClient {
    ...
    private func normalize(_ work: WorkMeta) -> WorkMeta {
        WorkMeta(
            ...
            coAuthorConfirmed: work.coAuthorConfirmed,
            secondCoAuthorFIO: work.secondCoAuthorFIO,
            secondCoAuthorUserName: work.secondCoAuthorUserName,
            secondCoAuthorConfirmed: work.secondCoAuthorConfirmed,
    ...
    private static func commentFromChunk(id: Int, level: Int, threadId: Int, chunk: String) -> WorkComment {
        ...
        let created = Self.firstMatch(#"data-time="([^"]+)""#, in: chunk)
            ...
            ?? Self.firstMatch(#"datetime="([^"]+)""#, in: chunk)
            ?? Self.firstMatch(#"datetime='([^']+)'"#, in: chunk)
            ?? Self.firstMatch(#"(?is)<time[^>]*datetime=["']([^"']+)["']"#, in: chunk)
            ?? Self.firstMatch(#"(?is)class="[^"]*comment-date[^"]*"[^>]*>([^<]+)<"#, in: chunk)
            ?? Self.firstMatch(#"(?is)class="[^"]*date[^"]*"[^>]*data-time=["']([^"']+)["']"#, in: chunk)
            ?? Self.firstMatch(#"(?is)<span[^>]*title=["']([^"']+)["'][^>]*class=["'][^"']*time"#, in: chunk)
